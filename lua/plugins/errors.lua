return {
  ...
  -- Load when an LSP attaches so diagnostics are available when needed
  event = "LspAttach",
  priority = 1000,
  config = function()
    require("tiny-inline-diagnostic").setup {
      ...
      options = {
        ...
        show_source = {
          enabled = true, -- Show which LSP produced the diagnostic
          if_many = false,
        },
        ...
        multilines = {
          ...
          severity = nil, -- No severity filtering
        },
        ...
        -- May cause visual artifacts; lower throttle if you notice flicker
        ...
        -- Ensure plugin attaches when LSP attaches and when diagnostics change
    ...
    -- Enable Neovim's virtual text so diagnostics are visible throughout the buffer
    -- Disable Neovim virtual_text to avoid duplicate messages (tiny-inline already shows inline diagnostics)
    ...
    -- Filter out unwanted clangd diagnostics (e.g., "Included header ... is not used directly")
    do
